...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const { count: trainingsThisMonth } = await supabase.from('trainings').select('*', { count: 'exact', head: true })
            .gte('scheduled_date', `${currentMonth}-01`).lt('scheduled_date', `${currentMonth}-32`);
        const { data: payments } = await supabase.from('payments').select('amount')
            .gte('payment_date', `${currentMonth}-01`).lt('payment_date', `${currentMonth}-32`);
        ...
        res.status(200).json({
            ...
            riders: { total: ridersCount || 0, newThisMonth: 0 },
            trainings: { thisMonth: trainingsThisMonth || 0, change: 0 },
            revenue: { thisMonth: revenue, change: 0 },
